...
- Enforces <50 words total while capturing main interview details
...
def analyze_case_note(note: str) -> Dict:
    ...
    # 1. Word count (strict <50 target per spec)
    words = note.split()
    word_count = len(words)
    word_score = max(0, 25 - abs(word_count - 35))  # Sweet spot ~35 words
...
def generate_improved_note(original_note: str, analysis: Dict) -> str:
    """Generate a compliant <50-word improved version (narrative only)."""
...
with col1:
    ...
    note_input = st.text_area(
        ...
        height=220,
        help="Target: <50 words total. Full sentences only. Cover as many SOAPIE elements as relevant."
    )
    
    word_count_live = len(note_input.split())
    st.caption(f"Current word count: **{word_count_live}** / 50 target")
...
with col2:
    ...
    if "analysis" in st.session_state:
        ...
        m1.metric("Words", a["word_count"], delta="✅ <50" if a["word_count"] <= 50 else "⚠️ Too long")
        ...
        with st.expander("📋 Detailed Feedback & Tips"):
            ...
            if a["word_count"] > 50:
                st.warning(f"**Length**: {a['word_count']} words exceeds 50-word target. Condense to core facts only.")
            if a["has_fragments"]:
...
if "improved" in st.session_state:
    st.divider()
    st.subheader("3. AI-Improved Version (Coherent, <50 words, Full SOAPIE)")
